...
public class MenuUtil {
    ...
    public List<MenuVo> buildTree(List<MenuVo> menus, List<MenuVo> all) {
        for(MenuVo menu:menus) {
            List<MenuVo> list = all.stream().filter(v -> (!("1".equals(v.getType())))&&menu.getMenuId().equals(v.getParentId())).collect(Collectors.toList());
            if (list.size() != 0) {
    ...
    public List<MenuVo> buildAllTree(List<MenuVo> menus, List<MenuVo> all) {
        for(MenuVo menu:menus) {
            List<MenuVo> list = all.stream().filter(v -> menu.getMenuId().equals(v.getParentId())).collect(Collectors.toList());
            if (list.size() != 0) {
                menu.setChildren(this.buildAllTree(list, all));
            }
        }
        return menus;
    }
